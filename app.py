...
def process_part2():
    ...
    wav_path = filename.replace(".webm", ".wav")
    os.system(f"ffmpeg -i {filename} -ac 1 -ar 16000 -sample_fmt s16 {wav_path}")
    ...
    logits = ctc_model(input_values).logits
    predicted_ids = logits.argmax(dim=-1)
    ipa_pred = processor.batch_decode(predicted_ids)[0].replace(" ", "")

    # Whisper transcription
    whisper_result = whisper_model.transcribe(wav_path, language="en")
    text = whisper_result["text"].strip()
    label_ipa = ei.convert(text).replace("ˈ", "").replace("ˌ", "").replace("*", "").replace(" ", "")

    # Scoring
    lcs_len = len(set(ipa_pred).intersection(set(label_ipa)))
    score = round(lcs_len / max(len(label_ipa), 1), 2)
